...
router.get('/:query', async function (req, res) {
  ...
  if (req.query.withCates) {
    const cateIds = await Product.aggregate([
      { $match: { $text: { $search: query.query } } },
      { $group: { _id: '$category', count: { $sum: 1 } } },
      { $sort: { count: -1 } },
    ]);
    const topCateIds = cateIds.splice(0, 11);
    const topCates = await Category.find({
      _id: { $in: topCateIds.map((c) => c._id) },
    }).lean();
    cateGroups = _.orderBy(
      [
        ...topCateIds.map((e) => ({
          ...
          count: e.count,
          name: e._id
            ? topCates.find((c) => c._id.toString() === e._id.toString())?.name
            : 'Unknown',
        })),
        cateIds.reduce(
          (result, e) => {
